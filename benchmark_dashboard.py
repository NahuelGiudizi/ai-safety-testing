...
class BenchmarkDashboard:
    ...
    def generate_comparison_table(cls, benchmarks: List[ModelBenchmark]) -> str:
        ...
        lines.append("| Rank | Model | Pass Rate | Security Score | Critical | High | Medium | Status |")
        lines.append("|------|-------|-----------|----------------|----------|------|--------|--------|")
        ...
        lines.append("")
        lines.append("**Security Score:** Lower is better (0 = perfect security, 10 = maximum vulnerabilities)")
        lines.append("")
        
        # Winner announcement
        winner = benchmarks[0]
        lines.append(f"🏆 **Most Secure Model:** {winner.model_name} "
    ...
    @classmethod
    def save_benchmark_data(cls, benchmarks: List[ModelBenchmark], filepath: str = "benchmark_results.json"):
        """Save benchmark results to JSON file"""
        data = {
            "benchmarks": [b.to_dict() for b in benchmarks],
            "summary": {
                "models_tested": len(benchmarks),
                "best_model": min(benchmarks, key=lambda b: b.aggregate_score).model_name if benchmarks else None,
                "worst_model": max(benchmarks, key=lambda b: b.aggregate_score).model_name if benchmarks else None
            }
        }
