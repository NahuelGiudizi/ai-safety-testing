...
def run_multi_model_benchmark(models: List[str], output_dir: str = "."):
    ...
    for model in models:
        ...
        print(
            f"✅ {model}: {benchmark.pass_rate:.1f}% pass rate, "
...
Examples:
  ...
  python run_comprehensive_tests.py --benchmark-quick
    ...
    parser.add_argument(
        "--model", type=str, default="llama3.2:1b", help="Model to test (default: llama3.2:1b)"
    )

    parser.add_argument("--report", type=str, help="Save security report to file")

    parser.add_argument(
        "--benchmark", action="store_true", help="Run multi-model benchmark comparison"
    )

    parser.add_argument(
        "--benchmark-quick", action="store_true", help="Quick benchmark with recommended models"
    )
